...
{
    BinarySelectionCell* finishedCell = (BinarySelectionCell*)[self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:self.tableView.numberOfSections-1]];
    [finishedCell snapToBinaryPosition:NO];
}
...
- (void) handleSwipeFrom:(UISwipeGestureRecognizer *)recognizer {
    ...
    if (!swipedCell.binaryPosition && recognizer.direction == UISwipeGestureRecognizerDirectionRight) { //Flip to tick
        ...
        if ([[[self cellDictionaryForPath:swipedIndexPath] valueForKey:@"Title"] isEqualToString:@"Finished"]) {
            [[self cellDictionaryForPath:swipedIndexPath] setValue:[NSNumber numberWithBool:NO] forKey:@"Value"];
            [swipedCell performSelector:@selector(setBinaryPosition:) withObject:[NSNumber numberWithBool:NO] afterDelay:0.5];
